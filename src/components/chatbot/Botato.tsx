...
const Botato: React.FC = () => {
  ...
  useEffect(() => {
    const savedMessages = sessionStorage.getItem('botato-chat-history');
    if (savedMessages) {
      try {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      sessionStorage.setItem('botato-chat-history', JSON.stringify(messages));
    }
  }, [messages]);
  ...
  return (
    <>
      ...
      {!isOpen && (
        <Button
          onClick={toggleChat}
          className="fixed bottom-4 right-4 h-14 w-14 rounded-full bg-mintGreen hover:bg-mintGreen/90 shadow-lg z-50 p-0"
          size="sm"
        >
